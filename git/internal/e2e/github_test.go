...
func TestGitHubE2E(t *testing.T) {
	...
	githubAppID, err = strconv.Atoi(appID)
	g.Expect(err).ToNot(HaveOccurred())

	installID := os.Getenv(githubAppInstallIDEnv)
	if installID == "" {
		t.Fatalf("could not read github app installation id")
	}
	githubAppInstallationID, err = strconv.Atoi(installID)
	g.Expect(err).ToNot(HaveOccurred())
	...
	givePermissionsToApp := func(repo gitprovider.OrgRepository) error {
		...
		_, _, err = githubClient.Apps.AddRepository(ctx, int64(githubAppInstallationID), ghRepo.GetID())
		if err != nil {
			return err
		}
			...
			if githubApp {
				...
				authOptions.ProviderOpts = &git.ProviderOptions{
					...
					GitHubOpts: []authgithub.OptFunc{
						...
						authgithub.WithInstllationID(githubAppInstallationID),
