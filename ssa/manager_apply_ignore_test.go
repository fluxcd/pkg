...
func TestApply_DriftIgnoreRules_OptionalFields(t *testing.T) {
	...
	t.Run("other controller orphans ignored field and flux adopts in-cluster value", func(t *testing.T) {
		...
		// the field becomes orphaned — no Apply manager owns it.
		// With adopt behavior, Flux adopts the in-cluster value into its payload,
		// preserving the value while retaining ownership.
		...
		// Flux re-apply with ignore rule should adopt the in-cluster value
		// since no other Apply manager owns it at the field level.
		...
		// Verify the in-cluster value was preserved and Flux now owns the field.
		existing = deployObject.DeepCopy()
		if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
			t.Fatal(err)
		}
		adoptedReplicas, found, _ := unstructured.NestedInt64(existing.Object, "spec", "replicas")
		if !found {
			t.Fatal("expected spec.replicas to exist after adopt")
		}
		if adoptedReplicas != replicas {
			t.Errorf("expected spec.replicas=%d (in-cluster value adopted), got %d", replicas, adoptedReplicas)
		}
	})
...
func TestApply_DriftIgnoreRules_CreateSkipsIgnore(t *testing.T) {
	...
	t.Run("subsequent update adopts in-cluster value when Flux is sole owner", func(t *testing.T) {
		// On update, the ignore rule should take effect only for drifted fields.
		// First, have another controller change key1 to introduce drift via
		// client-side edit (no SSA manager). Since Flux is the sole Apply owner,
		// the in-cluster value should be adopted into the payload.
		cmClone := cm.DeepCopy()
		...
		// Flux should still own data.key1 (adopted, not stripped) and the
		// in-cluster value should be preserved.
		existing := cm.DeepCopy()
		if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
			t.Fatal(err)
		}
		val, found, _ := unstructured.NestedString(existing.Object, "data", "key1")
		if !found || val != "changed-by-other" {
			t.Errorf("expected data.key1=%q (in-cluster value adopted), got %q (found=%v)", "changed-by-other", val, found)
		}
	})
}
...
func TestApply_DriftIgnoreRules_ClientSideEditConsequences(t *testing.T) {
	...
	t.Run("optional field adopts in-cluster value when Flux is sole owner", func(t *testing.T) {
		...
		// The in-cluster value (10) should be preserved via adopt, not reverted
		// to Flux's desired (2) or the API server default (1).
		existing = deploy.DeepCopy()
		if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
			t.Fatal(err)
		}
		...
		if replicas != 10 {
			t.Errorf("expected spec.replicas=10 (in-cluster value adopted), got %d", replicas)
		}

		// Flux should still own replicas (adopted, not stripped).
		fluxOwnsReplicas := false
		for _, mf := range existing.GetManagedFields() {
			if mf.Manager == manager.owner.Field && mf.Operation == metav1.ManagedFieldsOperationApply {
				if mf.FieldsV1 != nil && strings.Contains(string(mf.FieldsV1.Raw), "f:replicas") {
		...
		if !fluxOwnsReplicas {
			t.Errorf("expected Flux to still own spec.replicas (adopted, not stripped)")
		}
	})

	t.Run("required field adopts in-cluster value when Flux is sole owner", func(t *testing.T) {
		...
		// With adopt behavior, the apply should always succeed — the in-cluster
		// image value is copied into the payload, avoiding the missing-required-field error.
		entry, err = manager.Apply(ctx, deploy, ignoreImageOpts)
		if err != nil {
			t.Fatalf("expected apply to succeed with adopt, got error: %v", err)
		}
		if entry.Action != ConfiguredAction {
			t.Errorf("expected ConfiguredAction, got %s", entry.Action)
		}

		// The in-cluster image (6.5.0) should be preserved.
		existing = deploy.DeepCopy()
		if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
			t.Fatal(err)
		}
		...
		if c0["image"] != "ghcr.io/stefanprodan/podinfo:6.5.0" {
			t.Errorf("expected client-side image 6.5.0 to be preserved via adopt, got %v", c0["image"])
		}
	})
}

func TestApply_DriftIgnoreRules_FieldManagerCleanupInteraction(t *testing.T) {
	timeout := 60 * time.Second
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()

	id := generateName("drift-cleanup")
	ns := &unstructured.Unstructured{
		Object: map[string]interface{}{
			"apiVersion": "v1",
			"kind":       "Namespace",
	...
	// Configure ignore rules from the start — replicas is always ignored.
	ignoreReplicasOpts := DefaultApplyOptions()
	ignoreReplicasOpts.DriftIgnoreRules = []jsondiff.IgnoreRule{
		{
			Paths: []string{"/spec/replicas"},
	...
	// Create the deployment with ignore rules — Flux owns all fields including replicas=2.
	if _, err := manager.Apply(ctx, deploy, ignoreReplicasOpts); err != nil {
		t.Fatal(err)
	}
	...
	// Verify VPA owns replicas in-cluster.
	existing := deploy.DeepCopy()
	if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
		t.Fatal(err)
	}
	...
	// Now add FieldManagers cleanup to remove vpa-controller.
	// This simulates: "ignore VPA's replicas, but also clean up VPA's managed fields entry"
	opts := ignoreReplicasOpts
	opts.Cleanup = ApplyCleanupOptions{
	...
	// Trigger a non-ignored change so the apply actually fires.
	if err := unstructured.SetNestedField(deploy.Object, int64(80), "spec", "minReadySeconds"); err != nil {
		t.Fatal(err)
	}

	t.Run("cleanup removes manager then drift adopts in-cluster value", func(t *testing.T) {
		// After cleanupMetadata removes VPA's managed fields entry on the server,
		// computeDriftedPaths should see that no other Apply manager owns replicas
		// and adopt the in-cluster value (5) instead of stripping it.
		// Without the fix, existingObject is stale (still has VPA entry),
		// so the code incorrectly strips replicas.
		...
		// The in-cluster replicas value (5) should be preserved via adopt,
		// NOT stripped (which would orphan/default it).
		existing := deploy.DeepCopy()
		if err := manager.client.Get(ctx, client.ObjectKeyFromObject(existing), existing); err != nil {
			t.Fatal(err)
		}
		gotReplicas, found, _ := unstructured.NestedInt64(existing.Object, "spec", "replicas")
		if !found {
			t.Fatal("expected spec.replicas to exist after apply")
		}
		if gotReplicas != 5 {
			t.Errorf("expected spec.replicas=5 (in-cluster value adopted after manager cleanup), got %d", gotReplicas)
		}

		// VPA's managed fields entry should be gone (cleaned up).
		for _, mf := range existing.GetManagedFields() {
			if mf.Manager == "vpa-controller" && mf.Operation == metav1.ManagedFieldsOperationApply {
				t.Errorf("expected vpa-controller managed fields entry to be removed by cleanup")
			}
		}

		// Flux should own replicas (adopted into payload).
		fluxOwnsReplicas := false
		for _, mf := range existing.GetManagedFields() {
			if mf.Manager == manager.owner.Field && mf.Operation == metav1.ManagedFieldsOperationApply {
				if mf.FieldsV1 != nil && strings.Contains(string(mf.FieldsV1.Raw), "f:replicas") {
		...
		if !fluxOwnsReplicas {
			t.Errorf("expected Flux to own spec.replicas (adopted after manager cleanup)")
		}
	})
}
...
// isFieldOwnedByOtherApplyManager against an indexed sub-object claim under
...
func TestApply_DriftIgnoreRules_ContainerResources(t *testing.T) {
	...
	// this, Flux must release on the next reconcile because the path is in
	// the ignore set and another Apply manager owns it.
	...
	// the indexed sub-object pointer). isFieldOwnedByOtherApplyManager must
	// walk vpa-controller's managedFields entry across the
