...
func NewRecorder(mgr ctrl.Manager, log logr.Logger, webhook, reportingController string, opts ...RecorderOption) (EventRecorder, error) {
	return NewRecorderForScheme(mgr.GetScheme(), mgr.GetAnnotatedEventRecorder(reportingController), log, webhook, reportingController, opts...)
}
...
func NewRecorderForScheme(scheme *runtime.Scheme,
	eventRecorder events.AnnotatedEventRecorder,
	...
	r := &recorder{
		...
		client:                 httpClient,
		AnnotatedEventRecorder: eventRecorder,
		log:                    log,
	}
	for _, o := range opts {
		o(r)
