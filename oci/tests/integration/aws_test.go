...
// getClusterUserAWS returns the cluster user for kubeconfig auth tests.
func getClusterUserAWS(output map[string]*tfjson.StateOutput) (string, error) {
	clusterUser := output["aws_wi_iam_arn"].Value.(string)
	if clusterUser == "" {
		return "", fmt.Errorf("no EKS cluster user id in terraform output")
	}
