...
type MultiDigester struct {
	d map[godigest.Algorithm]godigest.Digester
}
...
func NewMultiDigester(algos ...godigest.Algorithm) (*MultiDigester, error) {
	d := make(map[godigest.Algorithm]godigest.Digester, len(algos))
	for _, a := range algos {
		if _, ok := d[a]; ok {
			continue
		}
		if !a.Available() {
			return nil, fmt.Errorf("%w: %s", godigest.ErrDigestUnsupported, a)
		}
		d[a] = a.Digester()
	}
...
func (w *MultiDigester) Digest(algo godigest.Algorithm) godigest.Digest {
	if d, ok := w.d[algo]; ok {
		return d.Digest()
	}
	return ""
