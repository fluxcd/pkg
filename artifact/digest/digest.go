...
var (
	...
	Canonical = godigest.SHA256
)

func init() {
	// Register SHA-1 algorithm for support of e.g. Git commit SHAs.
	godigest.RegisterAlgorithm(SHA1, crypto.SHA1)
}
...
func AlgorithmForName(name string) (godigest.Algorithm, error) {
	a := godigest.Algorithm(name)
	if !a.Available() {
		return "", fmt.Errorf("%w: %s", godigest.ErrDigestUnsupported, name)
	}
	return a, nil
}
