...
func (p Provider) NewRESTConfig(ctx context.Context, cluster string, opts ...auth.Option) (*auth.RESTConfig, error) {
	...
	for i, kc := range resp.Kubeconfigs {
		...
		addresses = append(addresses, fmt.Sprintf("'%s'", restConfig.Host))
		canonicalHost, err := auth.ParseClusterAddress(restConfig.Host)
		if err != nil {
	...
	// Microsoft Entra ID integration is enabled. Let's impersonate the
