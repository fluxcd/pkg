...
func (tc *FakeTokenCredential) GetToken(ctx context.Context, options policy.TokenRequestOptions) (azcore.AccessToken, error) {
	...
	// Embed the scope inside the context. This enables consumers of a Provider configured with
	// this credential, to verify that the desired scope was specified while fetching the token.
	val, ok := ctx.Value("scope").(*string)
	if ok {
		*val = options.Scopes[0]
	}

	return azcore.AccessToken{Token: fmt.Sprintf("%s", tc.Token), ExpiresOn: tc.ExpiresOn}, nil
}
