...
func TestProvider_NewRESTConfig(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			err:     `invalid AKS cluster ID: 'invalid-cluster-id'. must match ^/subscriptions/([^/]{36})/resourceGroups/([^/]{1,200})/providers/Microsoft\.ContainerService/managedClusters/([^/]{1,200})$`,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
